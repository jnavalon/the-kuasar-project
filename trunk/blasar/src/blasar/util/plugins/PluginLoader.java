...
public class PluginLoader {
            ...
            if (files.length > 0) {
                ...
                for (File file : files) {
                    try {
                        ...
                        blasar.Info.showError("Impossible to load plugins \n" + ex.getMessage());
                    }
                }
            }
        } catch (Exception ex) {
            blasar.Info.showError(ex.getMessage());
            return false;
        }
        return true;
    }
    ...
    private static File[] getFiles() {
        ...
        File dirPlugins = new File( Config.BLASAR.startDir + Config.BLASAR.PluginsDIR );
        try {
            if (dirPlugins.isDirectory() && dirPlugins.exists()) {
                File[] plugins = dirPlugins.listFiles(new FilenameFilter() {

                    public boolean accept(File dir, String name) {
                        return name.endsWith(Config.BLASAR.PluginEXT);
                    }
                });
                if(plugins.length>1)
